...
public class HttpProtocol extends IOProtocol implements IOProtocolHandler {
	...
	/*
	 * 这里什么时候会出异常 为什么要在这个地方做异常处理_lijianqiao
	 */
	/**
	 * 初始化
	 */
...
//	}
	...
	/**
	 * 检查 HTTP 请求是否符合要求
	 * 需要判task里的type和date是否为GET||POST 和  String
	 * @param task
	 */
	@Override
	...
	public IOResponse handleRequest(IOTask task) {
		...
		try {
			...
			// 向服务器提交数据 (POST方式)
			/**
			下面代码的条件是！（task.getParameter() instanceof String）注意
			应该是task.getParameter() instanceof MFormFile[]才行  _lijianqiao */
			if (isPOST && task.getParameter() != null && !(task.getParameter() instanceof String)) {
				...
				//接下来的数据全写到response的outputStream里面_lijianqiao
				...
				/*
				 * 如果不是拿Parameter的数据就是拿postdata的数据了
				 */
			...
			/**
			 * 206回应是接受到了一部分 会不会出差错 _lijianqiao 抱有疑问
			 */
	...
	public int handleResponse(IOResponse rsp) {
		...
		try {
			...
			if (task.getContentLength() == 0) { // 服务器返回数据为空
				...
				if (bout != null
						/*
						 * 为什么上面只有200才成功 现在200 || 206 都行  _lijqianqiao  unknow
						 */
